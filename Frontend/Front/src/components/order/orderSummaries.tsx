...
// Define clear interfaces for better type safety and readability
...
export default function OrderSummaries({ order, products }: OrderSummariesProps) {
  // Calculate subtotal using reduce for a cleaner approach
      ...
      <div className="p-3 p-md-5 bg-gray-100 rounded-2">
        <div className="d-block d-md-flex justify-content-between align-items-center mb-4">
          <div className="d-block">
            ...
            <p className="mb-0 text-dark mt-4 mt-md-0">
              Order Placed <b>{order.date}</b>
            </p>
          </div>
          <button className="btn btn-dark btn-sm">View invoice</button>
        </div>

        {/* Map directly to OrderCardProduct components */}
        {products.map((product) => {
          // Find the corresponding product details from the order
          const orderProduct = order.products.find(
            (op) => op.id === product.id
          );

          // Default values if no match is found (shouldn't happen with correct data)
          const status = orderProduct?.status || 'N/A';
          ...
          <div className="row">
            ...
            <div className="col-12 col-lg-3 mt-4 mt-lg-0">
              ...
              <div className="d-flex align-items-center"> {/* Added align-items-center for vertical alignment */}
                {/* Consider extracting this SVG into its own component for reusability */}
            ...
            <div className="col-12 col-lg-6">
              <OrderSummary subtotal={subtotal} textColor="" />
            </div>
          </div>
        </div>
      </div>
